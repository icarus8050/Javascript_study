...
* 오브젝트는 {} 또는 new 라는 키워드를 통해 생성할 수 있다.
* {} 로 선언하는 방법은 'object literal' 라고 한다.
* new 로 선언하는 방법은 'object constructor' 라고 한다.
* object 는 key 와 value 의 집합체이다.

```javascript
const obj1 = {};
const obj2 = new Object();
const foo = {name: 'hello', age: 2};
foo.gender = 'man';  // javascript 는 동적 타입 언어이므로 타입이 런타임에 결정되므로 프로퍼티도 동적으로 추가할 수 있다.
delete foo.gender;   // delete 키워드를 통해서 프로퍼티를 삭제할 수도 있다.
```

#### Computed properties

* 오브젝트의 프로퍼티에 접근할 때는 HashMap 에 접근하는 것처럼 프로퍼티명을 명시하여 접근할 수 있다.

```javascript
const person = {name: 'hello', age: 2};
console.log(person['name']) // hello
```

#### Property value shorthand

* 함수의 key 와 value 의 이름이 파라미터와 같다면 아래와 같이 key 를 생략하고 간단하게 오브젝트를 생성할 수 있다.

```javascript
const person1 = {name: 'bob', age: 2};
const person2 = makePerson('hello', 5);
